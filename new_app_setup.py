...
def main():
    ...
    parser.add_argument('--working_dir', type=str, required=False,
                        help='Location to store onboarding files.')

    parser.add_argument('--organization', type=str, required=True,
                        help='organization group')
    ...
    create_gcloud_service_account(team_name, working_dir, organization)
    ...
    create_rolebinding_yml(rolebinding_filename, team_name, organization)
...
def create_gcloud_service_account(team_name, working_dir, organization):
    ...
    subprocess.check_call(['gcloud', 'projects', 'add-iam-policy-binding',
                           organization, '--member', 'serviceAccount:' +
                           service_account_email, '--role',
                           'projects/'+organization +
                           '/roles/AppTeamServiceAccount'])
...
def create_rolebinding_yml(rolebinding_filename, team_name, organization):
    """
    ...
    yml_file_kubernetes_data = dict(
        ...
        subjects=[
            {
                'kind': 'User',
                'name': team_name+'@'+organization+'.iam.gserviceaccount.com',
...
def create_docker_repo(team_name, docker_username,
                       docker_password, organization):
    """
    ...
    team under the MVPStudio organization.
    """
    ...
    data = {"namespace": organization,
            "registry": "registry-1.docker.io",
            "image": organization+"/"+team_name,
            "build_in_farm": "true",
            "name": team_name,
